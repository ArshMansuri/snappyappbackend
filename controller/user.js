  ...
  try {
    ...
    if (password <= 6) {
      return res.status(400).json({
        success: false,
        message:
          "User validation failed: password: password must be at least 6 characters",
      });
    }
...
exports.followAndUnFollow = async (req, res) => {
  try {
    ...
    if (!user.followors.includes(req.user._id)) {
      // follow user
...
exports.msgUsers = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "msg user save successfully",
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
